...
maincompute xs kmer= (flt_disdup (map sort_second (groupBy grp_disdup (q_sort_str (break_str xs kmer 0)))))  
                          ...
                          then return ()
                                     ...
                                     if (retvalue ==[])
                                       then mainloop inh outh k 
                                                ...
                                                hPutStrLn outh (unlines (map (\x->(show(x)++" >"++tail(takestring1)++"  "++takestring2)) (make_unique (sort retvalue))))
                                                mainloop inh outh k           
