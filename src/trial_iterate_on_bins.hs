...
run_20_90  home i g  kmer c_min c_max= if (i==g) 
                            		       ...
                            		       let log_file=(home ++"/global/global_log.txt")
                                               ...
                                               --appendFile1 log_file (path++"\n:finished bucket "++show(i)++"\n")
                                               --exitId1<-system("cat adjacency_matrix.txt |grep -v \"Nothing\"|awk '{print $2\" \"$4}' >adjacency_matrix_1.txt")
                                               --contentb1<-readFile ("filter_sorted_kmer_frequency.txt")
                                               --content1<-system(path++"/bin/fastq_line_2 "++this_dir++"/"++show(i)++"_reads.fq "++this_dir++"/lined_reads.fq")
                                               content1<-system(path++"/bin/grep_reads "++this_dir++"/lined_reads.fq "++this_dir++"/filter_sorted_kmer_frequency.txt "++this_dir++"/1st_filter_reads.txt")
                                               --content1<-system("sort "++this_dir++"/1st_filter_reads.txt "++"-o "++this_dir++"/s_1st_filter_reads.txt")
                                               --content1<-system(path++"/bin/make_unique "++this_dir++"/s_1st_filter_reads.txt "++this_dir++"/u_1st_filter_reads.txt ")
                                               --content1<-system(path++"/bin/make_fq_pipeline "++this_dir++"/u_1st_filter_reads.txt "++this_dir++"/u_out_1st_filter_reads.fq") 
                                               --exitId<-system(path++"/bin/all_complement "++this_dir++"/u_out_1st_filter_reads.fq "++this_dir++"/both_reads.txt")
                                               --exitId<-system(path++"/bin/find_kmers_2_latest"++" "++this_dir++"/both_reads.txt "++kmer++" "++this_dir++" "++c_min++" "++c_max)
                                               ...
                                               --content1<-systeme(path++"/bin/fastq_line_2 "++this_dir++"/"++show(i)++"_reads.fq "++this_dir++"/lined_reads.fq")
...
samt::String --path of samtools executable  
samt="samtools" 

bamt::String --path of bamtools executable 
bamt="bamtools" 

phrap::String --path of phrap executable 
phrap="/home/nr8/Phred/phrap"

blastn::String --path of blast executable  
blastn="/hgsc_software/ncbi-blast-2.2.26+/bin/blastn" 

bdir::String --path of the reference from blasting against. It is necessary to prepare the reference files for running blast for this
bdir="/stornext/snfs6/cancer-analysis/users/navin/new-assembler/current_version/feb13/1st_run/PASRTP/secondary/analysis/ed/rptmask/blast/hg19.fa" 
 
intersectBed::String --path of intersectBed executable
intersectBed="/hgsc_software/BEDTools/latest/bin/intersectBed" 

path::String
path="/home/nr8/test_ITDASM"
